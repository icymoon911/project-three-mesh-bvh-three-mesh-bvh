(function (global, factory) {
	...
	function isTriangleValid$1( triStart, drawStart, drawEnd, groups ) {

		if ( triStart < drawStart || triStart + 3 > drawEnd ) return false;
		if ( groups && groups.length > 0 ) {

			for ( let g = 0, gl = groups.length; g < gl; g ++ ) {

				const gStart = groups[ g ].start;
				const gEnd = gStart + groups[ g ].count;
				if ( triStart >= gStart && triStart + 3 <= gEnd ) return true;
	...
	function refit( bvh, nodeIndices = null ) {
		...
		// Cache the draw range and groups so leaf traversal can skip triangles
		// that are outside the current renderable range.
		const drawRange = geometry.drawRange;
		const drawStart = drawRange.start | 0;
		const drawEnd = isFinite( drawRange.count ) ? drawStart + drawRange.count : Infinity;
		...
		function _traverse( nodeIndex32, byteOffset, force = false ) {
			...
			if ( IS_LEAF( nodeIndex16, uint16Array ) ) {
				...
				for ( let tri = offset, triEnd = offset + count; tri < triEnd; tri ++ ) {
					...
					const triStart = 3 * tri;

					// Skip triangles outside the draw range or in group gaps
					if ( ! isTriangleValid$1( triStart, drawStart, drawEnd, groups ) ) continue;

					for ( let j = 0; j < 3; j ++ ) {

						let index = indexArr[ triStart + j ];
	...
	const temp5$1 = /* @__PURE__ */ new three.Vector3();
	...
	function closestPointToGeometry(
		...
		if ( target2 ) {
			...
			// Use a dedicated temp vector (temp5) for the distance calculation so that
			// tempTargetDest1 (which backs target1.point in some paths) is not mutated
			// by applyMatrix4 or sub.
			temp5$1.copy( tempTargetDest1 ).applyMatrix4( tempMatrix$1 );
			target2.distance = temp5$1.distanceTo( target2.point );
	...
	// Check whether the triangle starting at index-buffer position `triStart`
	// falls within the geometry's current drawRange and (if present) any group.
	function isTriangleValid( triStart, drawStart, drawEnd, groups ) {
	...
	const temp5 = /* @__PURE__ */ new three.Vector3();
	...
	function closestPointToGeometry_indirect(
		...
		if ( target2 ) {
			...
			// tempTargetDest1 (which backs target1.point in some paths) is not mutated
			// by applyMatrix4 or sub.
			temp5.copy( tempTargetDest1 ).applyMatrix4( tempMatrix );
			target2.distance = temp5.distanceTo( target2.point );
			target2.faceIndex = closestDistanceOtherTriIndex;
	...
	const _cachedInverseMatrix = /* @__PURE__ */ new three.Matrix4();
